    ...
    X: Executable + 'static,
{
    ...
    // TODO[agg_v2]: clean control flow / unreachables here and in replace_ids_...
    fn map_id_to_values_in_write_set(
        resource_write_set: BTreeMap<T::Key, (T::Value, Option<Arc<MoveTypeLayout>>)>,
    ...
    fn materialize_txn_commit(
        ...
        for (key, (value, layout)) in last_input_output
            ...
            match res {
                ExecutionStatus::Success(output) | ExecutionStatus::SkipRest(output) => {
                    ...
                    if dynamic_change_set_optimizations_enabled {
                        ...
                        let mut patched_resource_write_set = Self::map_id_to_values_in_write_set(
